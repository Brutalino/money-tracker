import type { TranslationKeys } from './types'

export const en = {
  common: {
    save: 'Save',
    saveChanges: 'Save changes',
    cancel: 'Cancel',
    close: 'Close',
    back: 'Back',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    name: 'Name',
    emoji: 'Emoji',
    date: 'Date',
    noteOptional: 'Note (optional)',
    total: 'Total',
    loading: 'Loading...',
    saveFailed: 'Save failed. Please try again.',
    deleteFailed: 'Delete failed. Please try again.',
    category: 'Category',
    other: 'Other',
    all: 'All',
    reactivate: 'Reactivate',
    notAvailable: 'N/A',
    seeAll: 'See all',
    amountOfTarget: (amount, target) => `${amount} of ${target}`,
    perMonth: (amount) => `${amount}/month`,
  },
  nav: {
    ariaLabel: 'Main navigation',
    home: 'Home',
    expenses: 'Expenses',
    budget: 'Budget',
    savings: 'Savings',
    report: 'Report',
  },
  header: {
    settingsAriaLabel: 'Settings',
  },
  monthSelector: {
    prevMonth: 'Previous month',
    nextMonth: 'Next month',
  },
  finance: {
    income: 'Income',
    incomePlural: 'Income',
    expense: 'Expense',
    expensePlural: 'Expenses',
    monthNet: 'Net',
  },
  transactionRow: {
    categoryFallback: 'Category',
    generatedFromRecurring: 'Generated from a fixed cost',
    scheduled: 'Scheduled',
  },
  fab: {
    addTransaction: 'Add transaction',
  },
  keypad: {
    deleteAriaLabel: 'Delete',
    decimalAriaLabel: 'Decimal point',
  },
  pace: {
    good: "You're on track 👌",
    critical: "You're spending too fast, slow down 🚨",
    warning: "You're spending a bit too fast ⚠️",
    wayAhead: "Take it easy, you're well ahead 🙂",
  },
  charts: {
    noExpensesToShow: 'No expenses to show',
    noDataRecentMonths: 'No data for the last few months',
    noExpensesInCategory: 'No expenses in this category',
  },
  home: {
    balance: 'Balance',
    gaugeRemaining: (remainingPct) => `${remainingPct}% of income left`,
    gaugeEstimated: 'Estimated from your average income',
    gaugeOverspent: "Spent more than you earned: it'll carry over",
    gaugeBelowZero: (amount) => `You're ${amount} below zero`,
    chipIncomeLabel: 'Income',
    chipExpensesLabel: 'Expenses',
    chipSetAsideLabel: 'Set aside',
    paceTitle: "This month's pace",
    paceVariableLabel: 'Variable',
    paceProjection: (amount) => `At this pace you'll close the month around ${amount} in variable spending`,
    paceTooEarly: 'Just getting started: your pace will show up in a few days 🌱',
    budgetWatchTitle: 'Budgets to watch',
    budgetWatchAmounts: (spent, budget) => `${spent} € of ${budget} €`,
    createGoalInvite: 'Create a savings goal',
    goalSubLine: (saved, target, remaining) => `${saved} of ${target} · ${remaining} to go`,
    goalPace: (avg, month) => `At ${avg}/month you'll get there by ${month}`,
    goalReached: 'Reached 🎉',
    goalsMultiTitle: (total) => `Goals · ${total} saved in total`,
    goalsCompactAmounts: (saved, target) => `${saved} € of ${target} €`,
    seeAllGoals: (n) => `See all (${n}) ›`,
  },
  transactions: {
    noneTitle: 'No transactions',
  },
  goals: {
    noGoalsTitle: 'No goals yet',
  },
  quickEntry: {
    closeAriaLabel: 'Close',
    deleteAriaLabel: 'Delete',
    notePlaceholder: 'Add a note...',
    chooseCategory: 'Choose a category',
    enterAmount: 'Enter an amount',
    confirmDelete: 'Delete this transaction?',
  },
  spese: {
    fixedCostsHeader: (activeCount) => `Fixed monthly costs · ${activeCount} active`,
    addFixedCostAriaLabel: 'Add fixed cost',
    noFixedCosts:
      'No fixed costs set up yet. Add rent, subscriptions and other recurring expenses: they will be recorded automatically every month.',
    disabledSuffix: ' · disabled',
    searchPlaceholder: 'Search by note...',
    noTransactionsFilteredSubtitle: 'There are no transactions for this month or with these filters.',
  },
  frequency: {
    monthly: 'Monthly',
    bimonthly: 'Every 2 months',
    quarterly: 'Quarterly',
    annual: 'Yearly',
  },
  recurringForm: {
    editTitle: 'Edit fixed cost',
    newTitle: 'New fixed cost',
    expenseOption: 'Expense',
    incomeOption: 'Income',
    namePlaceholder: 'E.g. Rent, Netflix, Gym...',
    amountLabel: 'Amount (for the chosen frequency)',
    frequencyLabel: 'Frequency',
    startsLabel: 'Starts',
    startsThisMonth: 'This month',
    startsNextMonth: 'Next month',
    deactivate: 'Deactivate',
    confirmDelete: 'Delete this fixed cost? Transactions already generated will stay in the history.',
    confirmDeleteThisMonthTx: (amount) => `Also delete this month's transaction (${amount})?`,
    deleteFailedRetry: 'Delete failed. Please try again.',
  },
  budget: {
    totalBudget: 'Total budget',
    suggestFromHabits: '✨ From my habits',
    suggestWithTarget: '🎯 With a savings target',
    copyFrom: (month) => `📋 Copy from ${month}`,
    noBudgetToCopy: (month) => `No budget to copy from ${month}`,
    confirmCopy: (from, to) => `Copy the budgets from ${from} to ${to}?`,
    noCategoriesTitle: 'No expense categories',
    noCategoriesSubtitle: 'Add categories in settings to start setting a budget.',
    budgetForCategoryAriaLabel: (category) => `Budget for ${category}`,
    spent: (amount) => `Spent ${amount}`,
    remaining: 'Remaining',
    exceededBy: 'Over by',
    noBudget: 'No budget',
    suggestedBudgetTitle: 'Suggested budget',
    notEnoughHistory:
      "There isn't enough spending history from recent months to calculate a suggestion yet. Log a few expenses and try again later.",
    suggestExplanation: 'Calculated from the average spending of recent months, rounded to the nearest €5.',
    applyToMonth: 'Apply to this month',
  },
  risparmi: {
    balanceLabel: 'Available balance',
    balanceExplanation: 'Everything you have, minus what you have already set aside',
    alreadySetAside: (amount) => `${amount} already set aside this month`,
    setAside: 'Set aside',
    noGoalsSubtitle: 'Create your first savings goal: a trip, a purchase, an emergency fund.',
    newGoal: 'New goal',
    firstPeriodHint: (date) => `First month: tracking since ${date}. Totals may be partial.`,
  },
  goalCard: {
    reached: 'Goal reached! 🎉',
    deadlinePassed: 'Deadline passed',
    byDeadline: (month, amount) => `By ${month}: ${amount}/month`,
    projection: (month) => `At this pace you'll reach it by ${month}`,
    addContributionsForProjection: 'Add contributions to see a projection',
    editGoalAriaLabel: 'Edit goal',
    addContribution: '+ Add contribution',
    openContributionsAriaLabel: 'Contributions made',
  },
  goalForm: {
    editTitle: 'Edit goal',
    newTitle: 'New goal',
    nameLabel: 'Goal name',
    namePlaceholder: 'E.g. Vacation, New car...',
    targetLabel: 'Target (€)',
    deadlineLabel: 'Deadline (optional)',
    confirmDelete: 'Delete this goal and all its contributions?',
  },
  contributionSheet: {
    title: (goalName) => `Add to "${goalName}"`,
    amountLabel: 'Amount (€)',
  },
  contributionsList: {
    title: (goalName) => `Contributions to "${goalName}"`,
    empty: 'Nothing set aside for this goal yet',
    total: (amount) => `Total set aside: ${amount}`,
    deleteAriaLabel: 'Delete contribution',
    confirmDelete: 'Delete this contribution? The money goes back into your available balance.',
  },
  report: {
    savingsRate: 'Savings rate',
    avg6Months: '6-month average',
    expensesByCategory: 'Expenses by category',
    incomeVsExpenses6m: 'Income vs expenses (6 months)',
    categoryTrend: 'Category trend',
    sortAmount: 'Amount',
    sortDate: 'Date',
    expenseCount: (n) => (n === 1 ? '1 expense' : `${n} expenses`),
  },
  settings: {
    title: 'Settings',
    guideTitle: 'Guide',
    openGuide: 'Open the guide',
    feedbackTitle: 'Feedback',
    sendFeedback: 'Send feedback',
    appearance: 'Appearance',
    themeAuto: 'Auto',
    themeLight: 'Light',
    themeDark: 'Dark',
    language: 'Language',
    languageEnglish: 'English',
    languageItalian: 'Italiano',
    periodTitle: 'Money month',
    periodStartLabel: 'Month starts on',
    periodFirstDay: '1st of the month',
    periodDayN: (day) => `Day ${day}`,
    periodSheetTitle: 'Month start day',
    periodExplainer:
      'Choose the day your money month starts, for example the day your salary arrives. Home, budgets, stats and savings all follow it.',
    periodFootnote: 'In shorter months the start day adapts automatically (e.g. 31 → 28 February).',
    expenseCategories: 'Expense categories',
    incomeCategories: 'Income categories',
    archivedTag: 'Archived',
    newExpenseCategory: '+ New expense category',
    newIncomeCategory: '+ New income category',
    dataSection: 'Data',
    exportBackupJson: 'Export backup (JSON)',
    exportTransactionsCsv: 'Export transactions (CSV)',
    importBackupJson: 'Import backup (JSON)',
    dangerZone: 'Danger zone',
    deleteAllData: 'Delete all data',
    diagnostics: 'Diagnostics',
    invalidBackupFile: "Invalid file: this doesn't look like a Money Tracker backup.",
    confirmImport:
      'Importing this backup will replace ALL current data. This action cannot be undone. Continue?',
    importSuccess: 'Backup imported successfully.',
    readFileFailed: 'Could not read the selected file.',
    confirmDeleteAll1: 'Delete ALL data (transactions, budgets, goals, categories)? This cannot be undone.',
    confirmDeleteAll2: 'Are you sure? This is the final confirmation: all data will be permanently deleted.',
    deleteAllSuccess: 'All data has been deleted.',
    diagVersion: 'Version',
    diagStandalone: 'navigator.standalone',
    diagDisplayMode: 'display-mode',
    diagViewportSize: 'innerWidth × innerHeight',
    diagScreenSize: 'screen.width × screen.height',
    diagVisualViewportHeight: 'visualViewport.height',
    diagSafeAreaTop: 'safe-area-inset-top',
    diagSafeAreaRight: 'safe-area-inset-right',
    diagSafeAreaBottom: 'safe-area-inset-bottom',
    diagSafeAreaLeft: 'safe-area-inset-left',
  },
  categoryForm: {
    editTitle: 'Edit category',
    newTitle: 'New category',
    colorLabel: 'Color',
    reactivateCategory: 'Reactivate category',
    archiveCategory: 'Archive category',
    confirmDeleteCategory: 'Delete this category? Its monthly budgets will be removed too.',
    cannotDeleteInUse:
      "This category can't be deleted: it has transactions or fixed costs. Archive it instead to hide it while keeping the history.",
    flexibilityLabel: 'Flexibility',
    flexibilityEssential: 'Essential',
    flexibilityEssentialHint: "won't be cut",
    flexibilityFlexible: 'Flexible',
    flexibilityVeryFlexible: 'Very flexible',
    flexibilityVeryFlexibleHint: 'cut first',
    habitLabel: 'Vice 🚬',
    habitHint: 'Never cut by smart budget, but shows how much quitting would save',
  },
  smartBudget: {
    title: 'Save with a target',
    targetQuestion: 'How much do you want to save per month?',
    targetInputLabel: 'Monthly savings target (€)',
    linkGoalLabel: 'Link to a goal (optional)',
    linkGoalNone: 'None',
    goalPaceHint: (amount) => `Prefilled from this goal's deadline: ${amount}/month`,
    motivationLabel: 'Or, why are you saving? (optional)',
    motivationPlaceholder: "E.g. it's good to have money aside",
    continue: 'Continue',
    noIncomeTitle: 'No income found',
    noIncomeBody: 'Add your salary as a recurring income first, then come back to set a savings target.',
    absurdTitle: "That target doesn't add up",
    absurdMessage: (target, max) =>
      `To save ${target} you'd have to live on less than zero. Theoretical max is ${max}, and that means spending nothing at all.`,
    adjustTarget: 'Adjust target',
    noCutsNeededBanner: (slack) =>
      `Your target fits your current habits: no cuts needed, you even have ${slack} of slack.`,
    cutsAppliedBanner: (target) => `Here's how to fit ${target}/month of savings into your budget.`,
    insufficientBanner: (max, target) =>
      `With realistic cuts (max −30%) you can save up to ${max}/month, not ${target}.`,
    proceedWithMax: 'Proceed with this proposal',
    lowerTarget: 'Go back and lower the target',
    baselineLabel: 'Usual',
    proposedLabel: 'Proposed',
    categoryBudgetAriaLabel: (category) => `Proposed budget for ${category}`,
    habitInsight: (name, monthly, yearly) =>
      `🚬 If you quit ${name}: +${monthly}/month, +${yearly}/year toward your savings`,
    confirm: 'Confirm and apply to this month',
    noHistoryTitle: 'Not enough history',
    noHistoryBody:
      "There isn't enough spending history yet to build a savings-target budget. Log a few expenses and try again later.",
  },
  guide: {
    title: 'Guide',
    intro: 'A quick tour of every screen: how to log things, and what all the numbers mean.',
    sections: {
      gettingStarted: {
        title: 'Getting started',
        body: [
          'Money Tracker is a personal expense, budget and savings tracker in euros. Everything you record stays private to this device.',
          'Five tabs at the bottom get you around: Home, Expenses, Budget, Savings and Report. Tap the gear icon on any screen to open Settings.',
          "There's no account and no cloud: all your data lives in this installation of the app, nowhere else. Use Settings → Export regularly to keep a backup.",
          'Tip: install the app to your home screen (Share → Add to Home Screen on iOS) for a full-screen, app-like experience.',
        ],
      },
      startingOut: {
        title: 'Where do I start?',
        body: [
          "The app tracks money from the day you begin. It doesn't know (or need) your bank balance. An expense counts on the day you pay it, an income on the day it arrives. Nothing else is assumed.",
          'You can start any day, even mid-month. Your first month will just look incomplete: the leftover may even be negative if your salary arrived before you started, and averages or budget suggestions will be based on partial data. This fixes itself from your first full month.',
          "Don't log your current bank balance as a big initial income to make the numbers look right: it would inflate that month's income and skew statistics and smart-budget suggestions for months. If you have money already set aside, record it as a contribution to a savings goal instead.",
          'Bills and salaries that straddle your start date follow one simple rule: whatever is paid after you start counts in the month you pay it, even if it "belongs" to money earned before. After one full cycle everything lines up on its own.',
          'For the cleanest numbers from day one, start at the beginning of your month: the 1st, or your payday if you set a custom start day in Settings. Both work: calendar months, or payday to payday. Pick the one that matches how you actually live.',
          'One last thing, the most important: the accuracy of every number in the app depends entirely on logging your expenses correctly. The app shows what you record, nothing more. The trick is to log right away, as soon as you spend: two taps at purchase time quickly become a habit. Leaving it for the evening, from memory, is the surest way to miss some.',
        ],
      },
      home: {
        title: 'Home',
        body: [
          "Home opens with an arc: the colored share is how much of this month's income you've already committed, between spending and savings. The number in the middle is your real running balance, income minus expenses minus savings, including whatever carried over from previous months.",
          "Below that, a pace card tracks your variable spending against your budget (or your recent average, if you haven't set one) and projects roughly where you'll land by the end of the month.",
          "Then, when a budget category is running hot, a card flags it so you can catch it early; and a goals card keeps your savings progress in view, so you can jump straight to Savings.",
        ],
      },
      addTransaction: {
        title: 'Adding a transaction',
        body: [
          'Tap the round + button to open quick entry. Type the amount on the keypad, then choose Expense or Income.',
          'Pick a category, optionally add a short note and change the date (it defaults to today).',
          'To edit or delete a transaction later, just tap it wherever it appears: Home, Expenses, or anywhere else in the app.',
          'If you already know about a future expense, like a bill with a known debit date, log it right away with that date: it will show as "Scheduled" until the day arrives, and it counts in the month it\'s paid in.',
        ],
      },
      spese: {
        title: 'Expenses',
        body: [
          'The Expenses tab lets you browse any month: use the arrows next to the month label to move backward and forward.',
          'Transactions are grouped by day, most recent first. Use the search box to filter by note, or tap a category chip to show only that category.',
          'The fixed costs card at the top shows the combined monthly equivalent of your active recurring items: tap it to expand and see each one.',
        ],
      },
      recurring: {
        title: 'Fixed costs',
        body: [
          'Fixed costs are for things like rent, subscriptions or installments: anything that repeats. Each one is just an amount and a frequency (monthly, every 2 months, quarterly or yearly): deliberately no due dates.',
          'Every month, each active fixed cost is automatically recorded as a transaction on the 1st, and converted to a monthly equivalent for the fixed-costs total.',
          'When adding or reactivating a fixed cost, you can choose whether it starts this month or the next one.',
          'Pausing a fixed cost stops it from being recorded further. Reactivating it starts recording again from the current month onward: the months it was paused are never backfilled.',
          'Variable bills (like power or gas) don\'t belong here: the amount changes every time. When a bill arrives, log it as a normal expense dated with its debit date: it will show as "Scheduled" until that day.',
        ],
      },
      budget: {
        title: 'Budget',
        body: [
          'Set a spending limit per category, in whole euros, for each month. A progress bar fills up as you spend, changing color as you approach or exceed it.',
          '"From my habits" proposes a budget based on the average of your last 3 months. "With a savings target" works backward from how much you want to save (based on your income) and spreads the cuts across categories based on how flexible each one is.',
          'You can also copy the whole budget from the previous month with one tap.',
        ],
      },
      risparmi: {
        title: 'Savings',
        body: [
          'Create a goal with a target amount and, if you want, a deadline. Add contributions to it whenever you set money aside.',
          'The card at the top shows your available balance, including whatever carried over from previous months, so you know how much you could set aside right now.',
          "Each goal card shows your progress and your average contribution pace, so you can see whether you're on track to hit the deadline.",
          'The list icon on a goal card opens every contribution you made to it: delete one and that money goes straight back into your available balance. Deleting a goal deletes its contributions too.',
        ],
      },
      report: {
        title: 'Report',
        body: [
          "A donut chart breaks down this month's spending by category.",
          'Tap a category in the donut or its legend to see every expense in it for the month, sorted by amount or date.',
          'A bar chart compares income and expenses over the last 6 months, so you can spot trends at a glance.',
          'A trend line follows a single category over the same 6 months: pick which one from the dropdown.',
          'At the top, your savings rate for this month and the 6-month average.',
          'Load your Fineco bank statement to check that the month adds up: the card at the bottom compares it against what you logged and lets you fix any gaps in one tap.',
        ],
      },
      settings: {
        title: 'Settings',
        body: [
          'Edit your categories: name, emoji, color, and for expense categories, a flexibility level (essential, flexible, very flexible) used by the smart budget. A category with no transactions or fixed costs can be deleted for good; otherwise, archive it to hide it while keeping its history.',
          'Switch the app language between English and Italian, and choose light, dark or automatic theme.',
          'Your month, by default, runs from the 1st to the last day of the calendar month. You can pick any start day instead, for example the day your salary arrives, like the 26th, so your month runs from the 26th to the 25th of the next one. The whole app (Home, budgets, stats and savings) follows it and re-maps immediately when you change it.',
          'Back up your data as a JSON file (or restore from one), export your transactions as CSV, or wipe everything from the danger zone.',
        ],
      },
    },
  },
  welcome: {
    title: 'Welcome to Money Tracker',
    subtitle: 'Track your spending, set budgets, and save toward what matters, all on your phone.',
    bullet1: 'Log an expense or income in seconds',
    bullet2: 'Set budgets and savings goals',
    bullet3: 'All your data stays on your phone',
    openGuide: 'Open the guide',
    skip: 'Start right away',
  },
  whatsNew: {
    title: "What's new",
    subtitle: (version) => `You are now on version ${version}`,
    gotIt: 'Got it',
  },
  feedback: {
    title: 'Send feedback',
    hint: "Tell me what works, what doesn't or what you'd like to see. The message is anonymous and includes the app version.",
    placeholder: 'Write your feedback here',
    send: 'Send',
    sending: 'Sending…',
    thanks: 'Thank you! Your feedback has been sent.',
    error: 'Could not send. Check your connection and try again.',
  },
  finecoCheck: {
    cardTitle: 'Check against your account',
    cardBadge: 'FINECO',
    cardDescription: 'Load your bank statement to see if the month adds up.',
    uploadButton: 'Load statement (.xlsx)',
    parsing: 'Reading file…',
    verifiedOn: (date) => `Verified ✓ · ${date}`,
    backToReport: 'Report',
    unrecognizedTitle: 'File not recognized',
    unrecognizedBody: "This doesn't look like the Fineco movements export.",
    unrecognizedSteps: [
      'Open the Fineco website (not the app)',
      'Go to Conto > Movimenti and choose the period',
      'Export to Excel and load the file unmodified',
    ],
    chooseAnotherFile: 'Choose another file',
    notCoveredTitle: (month) => `${month} is missing from the file`,
    notCoveredSub: (fromDate, toDate) => `The file covers from ${fromDate} to ${toDate}`,
    verifyMonthButton: (month) => `Check ${month}`,
    okTitle: (month) => `${month}: it all adds up`,
    okSub: (nBank, nMatched) => `${nBank} account movements, ${nMatched} found in the app`,
    minorTitle: 'Almost all matches',
    minorSub: (matched, diffs) => `${matched} movements match · ${diffs} differences`,
    bigTitle: "Something doesn't add up",
    bigSubBankMore: (amount) => `The account shows ${amount} more in expenses than the app`,
    bigSubAppMore: (amount) => `The app shows ${amount} more in expenses than the account`,
    partialBanner: (date) => `The file only goes up to ${date}: later days can't be checked.`,
    nearMissSectionTitle: (n) => `Probably the same expense (${n})`,
    inAppLabel: 'In the app:',
    onAccountLabel: 'On the account:',
    nearMissTagSameDay: (diff) => `Same day · ${diff} difference`,
    nearMissTagGap: (days, diff) => `${days} day${days === 1 ? '' : 's'} apart · ${diff} difference`,
    onlyAppSectionTitle: (n) => `Only in the app (${n})`,
    openExpenseButton: 'Open the expense',
    onlyBankSectionTitle: (n) => `Only on the account (${n})`,
    addToAppButton: 'Add to the app',
    totalsTitle: 'Totals for the month',
    totalsExpensesLabel: 'Expenses',
    totalsIncomeLabel: 'Income',
    totalsAppLabel: 'App',
    totalsBankLabel: 'Account',
    totalsEqual: 'equal ✓',
    readingHelp:
      '"Only in the app" can be a cash expense: that\'s fine as is. "Only on the account" is an expense you probably forgot to log.',
    sourceBanner: (description, date) => `From the account: ${description} · ${date}`,
  },
} satisfies TranslationKeys
