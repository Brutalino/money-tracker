...
- **What's new popup** (`src/lib/changelog.ts` + `src/components/WhatsNewSheet.tsx`): one `CHANGELOG` entry per feature version (e.g. `0.8`), en+it; a patch with user-visible changes updates that entry's items and bumps `updatedIn` so it's shown again to anyone who missed the revision, a release with nothing user-visible leaves `updatedIn` alone and shows no popup; skipped on fresh installs.
